...
class LectioBot(Bot):
    ...
    async def send_weekly_schedule(self, guild, week: int, year: int) -> None:
        ...
        await self.get_channel(self.skema_id[guild.name]).send(
            f"**🔥🔥 Her kommer skemaet for uge {week} 🔥🔥**"
        )

        for i in range(1, 6):
            await self.send_skema(guild, i, week, year)

    def launch(self, token: str) -> None:
    ...
    async def send_skema(self, guild, dag: int, uge: int, år: int, channel = None):
        ...
        await self.send_embed({
            "title": f"**{LectioBot.KALENDER[dag-1]} uge {uge}**",
            ...
            "description": f"Her kan du se skemaet for **{LectioBot.KALENDER[dag-1]}** uge **{uge}** år **{år}**",
            "fields": {
                f"{i}": {
                    "value": f"{skema[i]}",
                    "inline": True
